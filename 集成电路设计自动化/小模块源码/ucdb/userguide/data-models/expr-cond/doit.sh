...
#
SRC=test.sv
FLAGS="-cover e"  # for expression coverage only
# FLAGS="-cover c"  # for condition coverage only
# FLAGS="-cover ec" # for expression and condition coverage both
...
SunOS)
    ...
    ;;
Win*|CYGWIN_NT*)
    CC="cl -c -Ox -Oy /MD -DWIN32 -I $MTI_HOME/include "
    LD="link "
...
vlog $FLAGS $SRC
vsim -coverage -c top -do "run -all; coverage save test.ucdb; quit"
# Report showing the expression truth table:
vcover report -details -code e -details -dump test.ucdb
if [ -x $UCDBDUMP ]
then
