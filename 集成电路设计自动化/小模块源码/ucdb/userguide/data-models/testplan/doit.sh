...
if [ "$1" = "clean" ] ; then
    ...
    rm -f *.dll *.exp *.lib *.obj *.sl *.o *.so *.ucdb testplan
...
Linux)
    ...
    LDLIB="testplan.o $MTI_HOME/$platform/libucdb.a"
    ;;
SunOS)
...
Win*|CYGWIN_NT*)
    ...
    LDLIB=-DEFAULTLIB:"`cygpath -m $MTI_HOME/$platform/libucdb.lib`"
    ;;
*)
...
vlog $FLAGS $SRC
vsim -c top -do "run -all; coverage save test.ucdb; quit"
$CC testplan.c
if [ "$LIBPATH" = "" ] ; then
    $LD testplan $LDLIB
else
    $LD testplan "$LDLIB" "$LIBPATH"
fi

testplan test.ucdb
vsim -c -viewcov test.ucdb -do "coverage analyze -plan / -r; quit"
if [ -x $UCDBDUMP ]
then
