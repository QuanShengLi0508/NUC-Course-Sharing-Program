...
if {$is64bit == 1 } {
	if {$tcl_platform(os) eq "SunOS"} {
		...
		quietly set LD "/usr/ccs/bin/ld -G -Bsymbolic -o"
		if {$platform eq "sunos5v9"} {
			quietly set CC "cc -xarch=v9 -xcode=pic32 -c -I$INSTALL_HOME/include"
		}
	} else {
		quietly set CC "gcc -g -c -m64 -Wall -ansi -pedantic -fPIC -I. -I$INSTALL_HOME/include "
		quietly set LD "gcc -shared -lm -m64 -Wl,-Bsymbolic -Wl,-export-dynamic -o "
	}
	quietly set UCDB_LD "gcc -ldl -lm -m64 -o "
} else {
	if {$tcl_platform(os) eq "SunOS"} {
		...
		quietly set LD "gcc -shared -lm -m32 -Wl,-Bsymbolic -Wl,-export-dynamic -o "
	}
	quietly set UCDB_LD "gcc -ldl -lm -m32 -o"
}
