...
#
SRC=test.sv
APP1=create_filehandles
APP2=dump_filehandles

if [ "$1" = "clean" ] ; then
    rm -f transcript *.wlf core* *.log workingExclude.cov
    rm -f *.dll *.exp *.lib *.obj *.sl *.o *.so *.ucdb $APP1 $APP2
...
Linux)
    ...
    LDLIB1="$APP1.o $MTI_HOME/$platform/libucdb.a"
    LDLIB2="$APP2.o $MTI_HOME/$platform/libucdb.a"
    ;;
SunOS)
...
Win*|CYGWIN_NT*)
    ...
    LDLIB1="`cygpath -d $MTI_HOME/$platform/libucdb.lib`"
    LDLIB2="`cygpath -d $MTI_HOME/$platform/libucdb.lib`"
    ;;
*)
...
vlog -cover s $FLAGS $SRC
vsim -c -coverage top -do "run -all; coverage save test.ucdb; quit"
$CC $APP1.c
if [ "$LIBPATH" = "" ] ; then
    $LD $APP1 $LDLIB1
else
    $LD $APP1 "$LDLIB1" "$LIBPATH"
fi
$CC $APP2.c
if [ "$LIBPATH" = "" ] ; then
    $LD $APP2 $LDLIB2
else
    $LD $APP2 "$LDLIB2" "$LIBPATH"
fi
$APP1
$APP2 test.ucdb
vcover report -byfile -details test.ucdb
exit 0
